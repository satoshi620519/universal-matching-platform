-- Canonical profile/category schema change previously maintained only in the API Prisma stream.
-- Keep the deploy-time database migration stream self-contained so a fresh Render database
-- receives every table required by the API.
