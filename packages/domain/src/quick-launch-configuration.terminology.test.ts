...
describe('QuickLaunch terminology publication', () => {
  ...
  it('publishes normalized purchaser terminology without changing stable keys', () => {
    const published = publishQuickLaunchConfiguration({ ...draft, terminology: { user: '  Member ', match: '   ' } }, 1, '2026-01-01T00:00:00.000Z');
    expect(published.terminology).toEqual({ user: 'Member' });
