...
export type MatchHistoryItem = { id:string; targetAccountId:string; decision:'like'|'pass'; idempotencyKey:string; createdAt:string };
export function listMatches(){return apiRequest<{items:MatchProfile[]}>('/matches');}
export function listMatchHistory(){return apiRequest<{items:MatchHistoryItem[]}>('/matches/history');}
export function decideMatch(input:{targetAccountId:string;decision:'like'|'pass';idempotencyKey:string}){return apiRequest<{mutual:boolean}>('/matches/decision',{method:'POST',body:JSON.stringify(input)});}
