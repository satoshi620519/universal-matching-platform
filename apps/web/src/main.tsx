...
import { getAuthenticatedAccount, register, signIn, verifyEmail, createConversation, listMessages, sendMessage, type Account, type Message } from './api';
import './styles.css';
...
function Dashboard({ account, loading, error, onSignOut }: { account: Account | null; loading: boolean; error: string; onSignOut: () => void }) {
 const [conversationId,setConversationId]=useState(''); const [participantId,setParticipantId]=useState(''); const [messages,setMessages]=useState<Message[]>([]); const [body,setBody]=useState(''); const [messageStatus,setMessageStatus]=useState('');
