...
export function AdministrativeModerationDirectory({ api }: { api: AdministrativeModerationApi }) {
  ...
  return <div className="card">
    ...
    {tab === 'reports' ? <table><thead><tr><th>Report</th><th>Target</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead><tbody>{(page.items as readonly AdministrativeModerationReport[]).map(x => <tr key={x.id}><td>{x.id}</td><td>{x.targetId}</td><td>{x.targetType}</td><td>{x.status}</td><td><select value={action} disabled={!!busy} onChange={e => setAction(e.target.value as ModerationActionType)}><option value="warning">Warning</option><option value="restrict-features">Restrict features</option><option value="restrict-communication">Restrict communication</option><option value="suspend">Suspend</option><option value="close-without-action">Close without action</option></select><input value={reasonCategory} disabled={!!busy} onChange={e => setReasonCategory(e.target.value)} placeholder="Reason category"/><button disabled={!!busy || !reasonCategory.trim()} onClick={() => mutate(x.id + '-action', () => api.applyAction(x.id, { targetId: x.targetId, action, reasonCategory }))}>Apply</button><button disabled={!!busy} onClick={() => mutate(x.id, () => api.openCase(x.id))}>Open case</button><button disabled={!!busy} onClick={() => mutate(x.id + '-triage', () => api.transitionReport(x.id, 'triaged'))}>Triage</button><button disabled={!!busy} onClick={() => mutate(x.id + '-dismiss', () => api.transitionReport(x.id, 'dismissed'))}>Dismiss</button></td></tr>)}</tbody></table>
      : <table><thead><tr><th>Case</th><th>Report</th><th>Status</th><th>Actions</th></tr></thead><tbody>{(page.items as readonly AdministrativeModerationCase[]).map(x => <tr key={x.id}><td>{x.id}</td><td>{x.reportId}</td><td>{x.status}</td><td><button disabled={!!busy} onClick={() => mutate(x.id + '-review', () => api.transitionCase(x.id, 'under-review'))}>Review</button><button disabled={!!busy} onClick={() => mutate(x.id + '-close', () => api.transitionCase(x.id, 'closed'))}>Close</button></td></tr>)}</tbody></table>}
