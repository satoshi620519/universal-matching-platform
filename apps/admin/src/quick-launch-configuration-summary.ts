...
export function summarizeQuickLaunchConfiguration(record: QuickLaunchPublishedRecord): QuickLaunchConfigurationSummary {
  ...
  const terminology = snapshot.terminology?.terms && typeof snapshot.terminology.terms === 'object' && !Array.isArray(snapshot.terminology.terms)
    ? Object.fromEntries(Object.entries(snapshot.terminology.terms as Record<string, unknown>).filter((entry): entry is [string,string] => typeof entry[1] === 'string' && entry[1].trim().length > 0).map(([key,value]) => [key,value.trim()]))
    : undefined;
  return {
