...
function parseIsoDate(value: unknown, field: string): Date | undefined {
...
function parseRole(value: unknown): AdministrativeRoleKey {
  if (typeof value !== 'string' || !roles.has(value as AdministrativeRoleKey)) {
    throw new BadRequestException('role is invalid');
  }
  return value as AdministrativeRoleKey;
}
...
export class AdministrativeRoleManagementController {
  ...
  async assign(
    ...
    @Body() body: { role?: unknown; effectiveAt?: unknown; expiresAt?: unknown },
    @Headers('authorization') authorization?: string,
    @Headers('x-request-id') requestId?: string,
  ) {
    if (!accountId.trim()) throw new BadRequestException('accountId is required');
    const effectiveAt = parseIsoDate(body?.effectiveAt, 'effectiveAt');
    const expiresAt = parseIsoDate(body?.expiresAt, 'expiresAt');
    ...
    const principal = await this.principalResolver.requireAuthenticated({
      authorization,
      requestId: requestId ?? 'administration-role-assign',
    });
    await this.roles.assign({ actorId: principal.accountId, accountId, role: parseRole(body?.role), ...(effectiveAt ? { effectiveAt } : {}), ...(expiresAt ? { expiresAt } : {}) });
  ...
  async revoke(
    ...
    @Body() body: { revokedAt?: unknown },
    @Headers('authorization') authorization?: string,
    @Headers('x-request-id') requestId?: string,
  ) {
    if (!accountId.trim()) throw new BadRequestException('accountId is required');
    const revokedAt = parseIsoDate(body?.revokedAt, 'revokedAt');
    const principal = await this.principalResolver.requireAuthenticated({
      authorization,
      requestId: requestId ?? 'administration-role-revoke',
    });
    return {
      revoked: await this.roles.revoke({
