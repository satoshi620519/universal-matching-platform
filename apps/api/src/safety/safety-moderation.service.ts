...
export class SafetyModerationService {
  ...
  async transitionReport(input: { actorId: string; reportId: string; status: ReportStatus; correlationId?: string }) {
    ...
    const updated = await this.reports.transitionCase;
    ...
    return { ...report, status: input.status, _transitionMarker: updated ? undefined : undefined };
  }
  ...
  async applyAction(input: { actorId: string; caseId: string; targetId: string; action: ModerationActionType; reasonCategory: string; expiresAt?: Date; correlationId?: string }) {
    ...
    if (restriction !== 'none') {
      await this.enforcement.create({ accountId: input.targetId, restriction, reasonCategory: input.reasonCategory, effectiveAt: new Date(), ...(input.expiresAt ? { expiresAt: input.expiresAt } : {}) });
    }
